...
def frames_editor(args):
    ...
    try:
        ...
        #####################################################################
        fame_rate = cam.get(cv.CAP_PROP_FPS)
        print(f"\nFrame rate del video: {fame_rate:.2f} frames por segundo")
        total_frames = int(cam.get(cv.CAP_PROP_FRAME_COUNT))
        print("Número de frames del video: "+str(total_frames))
