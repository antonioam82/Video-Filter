...
class app:
    def __init__(self):
        ...
        self.filter_method["values"]=["Bilateral Filter","Blur","Median Blur","fastNlMeansDenoisingColored","Filter2D","pyrDown","Sketcht"]
    ...
    def aply_method(self,fr):
        if self.filter_method.get() == "Bilateral Filter":
            ...
            edit = cv.pyrDown(fr)
        elif self.filter_method.get() == "Sketcht":
    ...
    def sketching(self,fr):
        ...
        pencilsketcht=cv.divide(gray_image,invertedblur,scale=256.0)
        return pencilsketcht
    ...
    def create_new_video(self):
        ...
        if len(self.frames_list) > 0:
            ...
            if self.question == "yes" and self.canceled == False:
                ...
                out = cv.VideoWriter('filteredVideo.mp4',cv.VideoWriter_fourcc(*'mp4v'), frame_rate, size)
                ...
                if 'VidAudioInfo.mp3' in os.listdir():
                    ffmpeg.concat(vid,audi,v=1,a=1).output(self.vid_name).run(capture_stdout=True,capture_stderr=True)
                    print('BOTH')
                else:
